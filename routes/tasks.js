...
router.get('/', async (req, res) => {
  try {
    ...
    if (role === 'employee') {
      // Сотрудник видит свои задачи + неназначенные задачи компании
      sql += ' AND (t.assigned_to=? OR (t.assigned_to IS NULL AND t.company_id=?))';
...
router.post('/', async (req, res) => {
  try {
    ...
    if ((role === 'pm' || role === 'admin') && assigned_to) {
      ...
      if (!assignee) return res.status(404).json({ error: 'Пользователь не найден.' });
      assignTo    = assigned_to;
      taskCompany = assignee.company_id;
    }

    // PM может создать задачу без назначения (assigned_to = null)
    if ((role === 'pm' || role === 'admin') && assigned_to === null) {
      assignTo = null;
    }
...
// POST /api/tasks/:id/take — сотрудник берёт задачу себе
router.post('/:id/take', async (req, res) => {
  try {
    ...
    if (role !== 'employee') return res.status(403).json({ error: 'Только сотрудники могут брать задачи.' });

    const task = await db.get('SELECT * FROM tasks WHERE id=?', [req.params.id]);
    if (!task) return res.status(404).json({ error: 'Задача не найдена.' });
    if (task.company_id !== companyId) return res.status(403).json({ error: 'Нет доступа.' });
    if (task.assigned_to !== null) return res.status(400).json({ error: 'Задача уже назначена.' });

    await db.run('UPDATE tasks SET assigned_to=?, status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?',
      [userId, 'doing', req.params.id]);
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (role === 'employee') { sql += ' AND (t.assigned_to=? OR t.assigned_to IS NULL)'; params.push(userId); }
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (role === 'employee') {
      if (task.assigned_to !== userId) return res.status(403).json({ error: 'Нет доступа.' });
      const { status } = req.body;
      if (!['todo','doing','done'].includes(status)) return res.status(400).json({ error: 'Неверный статус.' });
      await db.run('UPDATE tasks SET status=?,updated_at=CURRENT_TIMESTAMP WHERE id=?', [status, req.params.id]);
    } else {
      if (role === 'pm' && task.company_id !== companyId) return res.status(403).json({ error: 'Нет доступа.' });
      const { title, description, status, priority, deadline, assigned_to } = req.body;
      const validS = ['todo','doing','done'];
      const validP = ['low','medium','high'];
      await db.run(
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (role === 'employee') return res.status(403).json({ error: 'Нет доступа.' });
